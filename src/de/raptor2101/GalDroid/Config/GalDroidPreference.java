...
public class GalDroidPreference {
	...
	private static SQLiteDatabase createConnection(){
		...
		dataBase.execSQL("CREATE TABLE IF NOT EXISTS " + CONFIG_TABLE
				+ " (id integer primary key AUTOINCREMENT, name varchar(100) unique, type_name varchar(100), "
			    + "root_link varchar(255), security_token varchar(255));");
		dataBase.execSQL("CREATE TABLE IF NOT EXISTS " + CACHE_TABLE
				+ " (hash varchar(31) primary key, lastAccessed bigint, size bigint);");
	...
	public static List<String> getGalleryNames()
	{
		...
		synchronized (mContext) {
			...
			Cursor dbCursor = database.rawQuery("SELECT name FROM "
					+ CONFIG_TABLE, null);
	...
	public static GalleryConfig getSetupByName(String name)
	{
		synchronized (mContext) {
			SQLiteDatabase database = createConnection();
			Cursor dbCursor = database.rawQuery(
					"SELECT id, type_name,root_link,security_token FROM "
							+ CONFIG_TABLE, null);
			GalleryConfig returnValue = null;
			if (dbCursor.moveToFirst()) {
				returnValue = new GalleryConfig(dbCursor.getInt(0), name, dbCursor.getString(1),
