...
public abstract class WorkerTask<ParameterType, ProgressType, ResultType>
		implements WorkerTaskInterface {
	private static final String CLASS_TAG = "WorkerTask";
	...
	private class WorkerRunnable implements Runnable {
		...
		public void run() {
			...
			while (mRunning) {
				try {
					...
					try {
						WorkerTaskMessage<ResultType> messageBody = new WorkerTaskMessage<ResultType>(
								mCurrentCallable.mParameter, null);
						message = mMessageHandler.obtainMessage(MESSAGE_PRE_EXECUTION, messageBody);
						message.sendToTarget();
						
						Log.d(CLASS_TAG, String.format("Executing callable for %s",	mCurrentCallable.mParameter));
					...
					Log.d(CLASS_TAG, String.format("Invoke callback for %s", mCurrentCallable.mParameter));
					message.sendToTarget();
		...
		private WorkerCallable waitForCallable() throws InterruptedException {
			synchronized (mCallables) {
				Log.d(CLASS_TAG, "Looking for enqueued callables");
				while (mCallables.size() == 0) {
					mIsSleeping = true;
					Log.d(CLASS_TAG, "Waiting for enqueued callables");
					mCallables.wait();
				}
				mIsSleeping = false;
				Log.d(CLASS_TAG, "Returning enqueued callables");
	...
	protected void enqueue(ParameterType parameter) {
		if (!mIsStopping) {
			Log.d(CLASS_TAG, String.format("Enqueuing %s", parameter));
			synchronized (mCallables) {
				mCallables.add(new WorkerCallable(parameter));
				Log.d(CLASS_TAG, "Notify waiting WorkerThread");
	...
	public final void start() {
		if (mWorkerThread == null
				|| mWorkerThread.getState() == State.TERMINATED) {
			mIsStarted = false;
			mIsSleeping = false;
			mIsStopping = false;
			...
			Log.d(CLASS_TAG, "Starting WorkerThread");
			mWorkerThread.start();
		}
	}
	...
	protected abstract void onPostExecute(ParameterType parameter,
			ResultType result);

	protected void onExceptionThrown(ParameterType parameter,
			Exception exception) {
	}

	protected void onProgressUpdate(ParameterType parameter,
	...
	protected boolean isActive(ParameterType parameter) {
		...
		Log.d(CLASS_TAG, String.format("Status: %s", status));
		if (status != Status.RUNNING) {
			return false;
		}
		Log.d(CLASS_TAG, String.format("CurrentCallable %s Parameter %s", mWorkerRunnable.mCurrentCallable, parameter));
		return mWorkerRunnable.mCurrentCallable != null
				&& mWorkerRunnable.mCurrentCallable.mParameter
						.equals(parameter);
	}
}
